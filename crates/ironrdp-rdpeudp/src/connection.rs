...
impl RdpeudpConnection {
    ...
    /// The protocol version the handshake settled on, once it is complete:
    /// 1 or 2 for the MS-RDPEUDP data transfer, 3 for MS-RDPEUDP2.
    pub fn negotiated_version(&self) -> Option<u16> {
        self.params.as_ref().map(|p| match p.wire {
            WireFormat::V1 { version } => version,
            WireFormat::V2 => 3,
        })
    }
